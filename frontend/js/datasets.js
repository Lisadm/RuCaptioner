...
const Datasets = {
    ...
    bindEvents() {
        ...
        // Prompt template change - show/hide custom prompt
        document.getElementById('captionSetTemplate')?.addEventListener('change', (e) => {
        ...
        // Edit caption set template change - show/hide custom prompt
        document.getElementById('editCaptionSetTemplate')?.addEventListener('change', (e) => {
        ...
        document.getElementById('datasetCaptionTemplate')?.addEventListener('change', (e) => {
            ...
            const manualContainer = document.getElementById('datasetCaptionManualContainer');
    ...
    async createCaptionSet() {
        ...
        const templateVal = document.getElementById('captionSetTemplate').value;
        ...
        // Map merged template to style/templateId
        let style = 'natural';
        let templateId = null;
        ...
        let seed = parseInt(document.getElementById('captionSetSeed').value);
        const seedMode = document.getElementById('captionSetSeedMode').value;

        // Handle seed modes for UI feedback
        if (seedMode === 'random') {
            seed = Math.floor(Math.random() * 1000000000);
            document.getElementById('captionSetSeed').value = seed;
    ...
    async showCaptionEditor(fileId) {
        ...
        try {
            ...
            document.getElementById('datasetCaptionPreview').src = imageUrl;
            ...
            // Russian translation
            document.getElementById('datasetCaptionRuText').value = data.caption?.caption_ru || '';
            ...
            // Load available vision models and restore last used
    ...
    async saveCaption() {
        ...
        try {
            ...
            const payload = {
                file_id: fileId,
                text: text,
                caption_ru: document.getElementById('datasetCaptionRuText').value.trim() || null,
                source: 'manual'
            };
    ...
    /**
     * Load available vision models into dropdown
     */
    async loadVisionModels() {
        const modelSelect = document.getElementById('datasetCaptionModel');
        if (!modelSelect) return;

        try {
            const models = await API.request('/vision/models');
            const lastUsedModel = localStorage.getItem('lastUsedVisionModel');

            modelSelect.innerHTML = '';

            // Add "Use default from settings" option
            const defaultOption = document.createElement('option');
            defaultOption.value = '';
            defaultOption.textContent = '(Default from settings)';
            modelSelect.appendChild(defaultOption);

            if (models.length > 0) {
                // Add each model as an option (show all, mark unavailable)
                models.forEach(model => {
                    const option = document.createElement('option');
                    option.value = model.backend_model_name;
                    ...
                    if (model.is_available) {
                        option.textContent = `${displayName} (${model.backend})`;
                    } else {
                        option.textContent = `${displayName} (${model.backend}) - not installed`;
            ...
            if (lastUsedModel) {
                const optionExists = Array.from(modelSelect.options).some(opt => opt.value === lastUsedModel && !opt.disabled);
                if (optionExists) {
            ...
            // On error, still show default option
            modelSelect.innerHTML = '<option value="">(Default from settings)</option>';
        }
    },
    ...
    async generateCaption() {
        ...
        try {
            ...
            // Get selected model and template from dropdowns
            const modelSelect = document.getElementById('datasetCaptionModel');
            const selectedModel = modelSelect?.value || null;

            const templateSelect = document.getElementById('datasetCaptionTemplate');
            const templateId = templateSelect?.value !== 'default' ? templateSelect?.value : null;

            // Handle seed logic
            const seedInput = document.getElementById('datasetCaptionSeed');
            const seedModeSelect = document.getElementById('datasetCaptionSeedMode');
            ...
            if (seedMode === 'random') {
                ...
                // We'll increment AFTER the successful call usually, 
                // but setting it here so the user sees what's being used.
                // Or better, increment it now.
                seed = seed + 1;
                seedInput.value = seed;
            } else if (isNaN(seed)) {
                seed = null;
            ...
            // For refine template, send current text as custom_prompt
            // For manual template, send user's custom instruction
            let generationPrompt = customPrompt;
            if (templateId === 'refine') {
                generationPrompt = document.getElementById('datasetCaptionText').value;
            } else if (templateId === 'manual') {
                const manualPrompt = document.getElementById('datasetCaptionManualPrompt')?.value?.trim();
                if (!manualPrompt) {
                    Utils.showToast('Please enter a custom instruction for manual mode', 'warning');
                    btn.disabled = false;
                    btn.innerHTML = originalHtml;
                    return;
                }
                generationPrompt = manualPrompt;
            }
            ...
            if (result.vision_model) {
                localStorage.setItem('lastUsedVisionModel', result.vision_model);
            }

            // Put the generated caption in the textarea
            const textEl = document.getElementById('datasetCaptionText');
            textEl.value = result.caption;
            document.getElementById('datasetCaptionRuText').value = result.caption_ru || '';
            this.updateCaptionCharCount();
    ...
    /**
     * Translate caption text bidirectionally (auto-detect language)
     * Shows translation in the Translation (Russian) field
     */
    async translateCaption() {
        const textEl = document.getElementById('datasetCaptionText');
        const ruTextEl = document.getElementById('datasetCaptionRuText');

        const sourceText = textEl.value.trim();

        if (!sourceText) {
            Utils.showToast('Введите текст в верхнее поле', 'warning');
            return;
        }

        // Top-Down Logic: Input is ALWAYS Top Box
        // If Top has Cyrillic -> RU to EN
        // Else -> EN to RU
        // Output -> ALWAYS Bottom Box

        const hasCyrillic = /[а-яА-ЯёЁ]/.test(sourceText);
        const direction = hasCyrillic ? 'ru_to_en' : 'en_to_ru';
        const targetEl = ruTextEl;
        const textToTranslate = sourceText;

        const btn = document.getElementById('datasetCaptionTranslateBtn');
        const originalHtml = btn.innerHTML;
        btn.disabled = true;
        ...
        try {
            ...
            const result = await API.request('/vision/translate', {
                method: 'POST',
                body: {
            ...
            // Update target field
            targetEl.value = result.translated_text;

            // Trigger auto-resize if it's a textarea
            targetEl.style.height = 'auto';
            targetEl.style.height = targetEl.scrollHeight + 'px';

            const dirLabel = direction === 'ru_to_en' ? 'RU→EN' : 'EN→RU';
            Utils.showToast(`${dirLabel}: Переведено за ${result.processing_time_ms}ms`, 'success');

        } catch (error) {
            Utils.showToast('Ошибка перевода: ' + error.message, 'error');
        } finally {
            btn.disabled = false;
            btn.innerHTML = originalHtml;
        }
    ...
    async editCaptionSet(captionSetId) {
        try {
            ...
            // Set template value (prioritize template_id then style)
            let templateVal = captionSet.template_id || captionSet.style || 'natural';

            // Fallback for legacy styles (Natural, Detailed, Tags) to the new curated "detailed_p"
            if (['natural', 'detailed', 'tags'].includes(templateVal)) {
                templateVal = 'detailed_p';
            }

            document.getElementById('editCaptionSetTemplate').value = templateVal;
            ...
            // Show/hide custom prompt based on selected value
            document.getElementById('editCustomPromptGroup').style.display =
                templateVal === 'custom' ? 'block' : 'none';
    ...
    async saveCaptionSet() {
        ...
        const templateVal = document.getElementById('editCaptionSetTemplate').value;
